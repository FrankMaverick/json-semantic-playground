...
def validate(fpath: Path, errors):
    ...
    if filename not in path_dirs.parts:
        log.debug(f"Filename '{fpath.name}' in path '{fpath}' does not match its containing directory name")
        errors.append(f"Filename '{fpath.name}' in path '{fpath.as_posix()}' does not match its containing directory name")
        return False
