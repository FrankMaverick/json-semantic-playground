...
def validate(fpath: Path, errors: list):
    ...
    for dir_path in dirs:
        ...
        if not re.match(pattern, dir_name):
            log.debug(f"The name of the directory '{dir_name}' in path '{dir_path}' does not match the required pattern")
            errors.append(f"The name of the directory '{dir_name}' in path '{dir_path}' does not match the required pattern")

    # Check the name of the file    
    if not re.match(pattern, fpath.stem):
        log.debug(f"The name of the file '{fpath.name}' in path '{fpath.parent}' does not match the required pattern")
        errors.append(f"The name of the file '{fpath.name}' in path '{fpath.parent}' does not match the required pattern")  
