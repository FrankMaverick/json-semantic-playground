...
def validate(fpath: Path, errors: list):
    ...
    if set(subdirs) <= set(required_subdirs):
        ...
        if missing_dirs:
            log.debug(f"One or more directories do not conform to the expected structure in '{root_dir}' dir: {missing_dirs}")
            errors.append(f"One or more directories do not conform to the expected structure in '{root_dir}' dir: {missing_dirs}")
